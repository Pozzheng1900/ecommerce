              ...
              <tbody>
                @forelse ($cart_items as $item)
                  <tr wire:key='{{$item['product_id']}}'>
                    ...
                    <td class="py-4">{{Number::currency($item['unit_amount'], 'KHR')}}</td>
                    <td class="py-4">
                      <div class="flex items-center">
                          <button wire:click='decreaseQty({{$item["product_id"]}})' 
                              class="border rounded-md py-2 px-4 mr-2 bg-gray-100 hover:bg-red-500 hover:text-white transition">
                              -
                          </button>
                    ...
                    <td class="py-4">{{Number::currency($item['total_amount'], 'KHR')}}</td>
                    <td>
                ...
                @empty
                  <tr>
                    <td class="text-center py-4 text-xl font-semibold text-slate-500" colspan="5">No items in the cart</td>
                  </tr>
                @endforelse
                <!-- More product rows -->
              </tbody>
        ...
        <div class="md:w-1/4">
          <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="flex justify-between mb-2">
              <span>Subtotal</span>
              <span>{{Number::currency($grand_total, 'KHR')}}</span>
            </div>
            <div class="flex justify-between mb-2">
              <span>Taxes</span>
              <span>{{Number::currency(0, 'KHR')}}</span>
            </div>
            <div class="flex justify-between mb-2">
              <span>Shipping</span>
              <span>{{Number::currency(0, 'KHR')}}</span>
            ...
            <div class="flex justify-between mb-2">
              ...
              <span class="font-semibold">{{Number::currency($grand_total, 'KHR')}}</span>
            </div>
            @if ($cart_items)
