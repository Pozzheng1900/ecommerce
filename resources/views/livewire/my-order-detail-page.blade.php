            ...
            <div class="mt-1 flex items-center gap-x-2">
              @php
                    ...
                    if($order->payment_status == 'pending') {
                      $payment_statue = '<span class="bg-yellow-500 py-1 px-3 rounded text-white shadow">Pending</span>';
                    }
                    if($order->payment_status == 'paid') {
                      $payment_statue = '<span class="bg-green-500 py-1 px-3 rounded text-white shadow">Paid</span>';
                    }

                    if($order->payment_status == 'failed') {
                      $payment_statue = '<span class="bg-red-500 py-1 px-3 rounded text-white shadow">Failed</span>';
                    }
                  @endphp
              <div>{!! $status !!}</div>
            </div>
            ...
            <tbody>
              @foreach ($order_items as $item)
                <tr wire:key="{{$item->id}}">
                  ...
                  <td class="py-4">
                    {{Number::currency($item->unit_amount, 'KHR')}}
                  </td>
                  <td class="py-4">
                  ...
                  <td class="py-4">
                    {{Number::currency($item->total_amount, 'KHR')}}
                  </td>
                </tr>
              @endforeach 
            </tbody>
      ...
      <div class="md:w-1/4">
        <div class="bg-white rounded-lg shadow-md p-6">
          ...
          <div class="flex justify-between mb-2">
            <span>Subtotal</span>
            <span>
              {{Number::currency($order->grand_total, 'KHR')}}
            </span>
          </div>
          <div class="flex justify-between mb-2">
            <span>Taxes</span>
